...
var loginWithMeraID = $('#loginWithMeraID');

var baseURI = 'http://localhost:3000'; //TODO update this
...
var getUserDetails = function () {
  ...
  $.ajax({
    ...
    .done(function (response) {
      console.log('response inside .done function', response);
      user = response;
      helloMessage.html('Hello ' + JSON.parse(response));

      console.log('response name inside ajx ', JSON.parse(response));
  ...
  makeAjaxCall('/auth/user/polls', 'get', {}, (err, resp) => {
    if (err) {
      ...
      for (var i = 0; i < resp.length; i++) {
        myPollList.append(
          '<a href="#" class="list-group-item my-polls id="' +
            resp[i]._id +
  ...
  makeAjaxCall('/poll', 'post', { poll_id: pollId }, (err, resp) => {
    if (err) {
      ...
      for (var i = 0; i < resp.options.length; i++) {
        ...
        if (i != 0)
          voteOptions.append(
            '<div type="radio"><label><input type="radio" name="optradio" value="' +
              i +
              '">' +
              '   ' +
              opt.option +
          ...
          voteOptions.append(
            '<div type="radio"><label><input type="radio" name="optradio" checked="true" value="' +
              i +
              '">' +
              '   ' +
              '   ' +
      ...
      voteForm.submit((e) => {
        e.preventDefault();
        var radioValue = $(
          "#vote-options input[name='optradio']:checked"
        ).val();
        makeAjaxCall(
          '/auth/vote',
          'post',
          { poll_id: pollId, option: radioValue },
          (err, response) => {
            if (err) {
...
function setUpNewPollContainer() {
  ...
  $(new_poll_form).keyup(function () {
    if (checkIfFilledOptions()) submit.prop('disabled', false);
  ...
  new_poll_form.on('submit', function (e) {
    ...
    if (optionss.length == $('.option').length) {
      makeAjaxCall(
        '/auth/new_poll',
        'POST',
        { name: name_, options: JSON.stringify(optionss) },
        (err, response) => {
          if (err) alert('error');
          else {
            alert(
              'Poll posted successfully at ' +
                baseURI +
                '/?vote=' +
                response.poll['_id']
            );
